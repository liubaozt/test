<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/ApproveEntry.Master"
    Inherits="System.Web.Mvc.ViewPage<BusinessLogic.AssetsBusiness.Models.AssetsInsure.EntryModel>" %>
...
<fieldset>
    <div class="editor-field">
        ...
        <%:Html.AppTextBoxFor(m => m.AssetsInsureNo, Model.PageId, "NoAssetsInsureEntry")%>
          <%:Html.AppNormalButton(Model.PageId, "btnAutoNo", "ui-icon-pencil", "autoNoButton")%>
        <%:Html.AppRequiredFlag()%>
        <%:Html.ValidationMessageFor(m => m.AssetsInsureNo)%>
    </div>
    <div class="editor-field">
        ...
        <%:Html.AppRequiredFlag()%>
        <%:Html.ValidationMessageFor(m => m.AssetsInsureName)%>
    </div>
    <div class="editor-field">
        <%:Html.AppLabelFor(m => m.InsureDate, Model.PageId, "AssetsInsureEntry")%>
        <%:Html.AppDatePickerFor(m => m.InsureDate, Model.PageId, "AssetsInsureEntry")%>
         <%:Html.ValidationMessageFor(m => m.InsureDate)%>
    </div>
     <%:Html.AppHiddenFor(m => m.AssetsInsureId, Model.PageId)%>
    <%:Html.AppHiddenFor(m=>m.EntryGridId,Model.PageId)%>
    </fieldset>
    <%:Html.AppEntryGridFor(this.Url, Model.PageId, Model.EntryGridId, Url.Action("EntryGridData", new { formMode = Model.FormMode, primaryKey = Model.AssetsInsureId }), Model == null ? null : Model.EntryGridLayout, 300, 0, "btnSave", "AssetsInsure")%>
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="HeadContent" runat="server">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#btnAutoNo' + pageId).click(function () {
                $.ajax({
                    ...
                    success: function (data) {
                        $('#AssetsInsureNo' + pageId).val(data);
                    }
                });
                return false;
            });
            ...
            //#region grid操作
            onCellEditAssetsInsure = function (id) {
